...
export const Icon = React.forwardRef((props, ref) => {
    ...
    if (iconPaths == null) {
        ...
        return (React.createElement(SVGIconContainer
        // don't forward Classes.iconClass(icon) here, since the container will render that class
        , { 
            // don't forward Classes.iconClass(icon) here, since the container will render that class
            className: classNames(Classes.intentClass(intent), className), color: color, iconName: icon, size: size, tagName: tagName, title: title, htmlTitle: htmlTitle, ref: ref, svgProps: svgProps, ...removeNonHTMLProps(htmlProps) }, pathElements));
    }
});
Icon.defaultProps = {
    autoLoad: true,
